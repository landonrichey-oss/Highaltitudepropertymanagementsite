...
// ✅ Mobile swipe works (iPhone + Android)
// ✅ X close button ALWAYS shows + works
// ✅ Gallery auto-renders ALL properties
// ✅ Modal auto-detects images (no counting needed)
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // ✅ Image folder base (use relative paths that work on Netlify)
  ...
  async function getPropertyImages(prop) {
    // Manual override support if ever needed
  ...
  // ✅ No cover fields needed — cover auto uses slug-1.jpg
  // =============================
  const properties = [
    {
      slug: "Rustic-Roots",
    ...
    // ✅ slug matches image pattern Bright-Mountain-Retreat-1.jpg etc
    {
    ...
    properties.forEach((prop, index) => {
      ...
      card.innerHTML = `
        <img
          ...
          role="button"
          tabindex="0"
          aria-label="View details for ${prop.title || "property"}"
        />
        <div class="card-body">
          <h3>${prop.title || ""}</h3>
      ...
      setImgFallback(card.querySelector("img"));
  ...
  // =============================
  // ✅ Render Gallery (auto from properties)
  // =============================
  function renderGallery() {
    ...
    properties.forEach((prop, index) => {
      ...
      block.innerHTML = `
        <button type="button" class="gallery-item" data-open-modal="${index}" aria-label="View ${prop.title || "property"} details">
          <img src="${cover}" alt="${prop.title || "Property"}" loading="lazy" />
        </button>
        <button type="button" class="gallery-overlay-link" data-open-modal="${index}">View Details</button>
      `;
  ...
  // ✅ Wrap each slide so it ALWAYS takes full width (prevents “half next photo”)
  ...
  function update() {
    // ✅ Always slide exactly 100% per image
  ...
  // ✅ SWIPE (Pointer events + Touch fallback) iPhone + Android
let startX = 0;
let startY = 0;
let dx = 0;
let dy = 0;
...
function onMove(x, y, e) {
  ...
  // If horizontal swipe, prevent scroll so the swipe registers
  if (locked === "x") {
    e.preventDefault();
  }
}
...
viewport.addEventListener("touchend", onEnd, { passive: true });
  ...
  // ✅ Async open (auto-detect photos)
  async function openPropertyModal(index) {
    const prop = properties[index];
    if (!prop || !modal) return;

    // Fill text + buttons immediately
    ...
    // ✅ Detect real images and rebuild slider,
    // BUT NEVER replace the cover with fallback
    const detected = await getPropertyImages(prop);

    const merged = uniqueImages([
      cover,
      ...(detected || []).filter((u) => u && u !== FALLBACK_IMG),
    ]);

    // If detection found nothing, keep cover only
    buildSlider(merged.length ? merged : [cover], prop.title || "Property");
  }

  // ✅ Property cards: click photo OR View details button opens modal
  propertyGrid?.addEventListener("click", (e) => {
    if (e.target.closest("a")) return;
    const trigger = e.target.closest("[data-property]");
    if (!trigger) return;
    openPropertyModal(Number(trigger.dataset.property));
  });

  // ✅ Keyboard support on image
  propertyGrid?.addEventListener("keydown", (e) => {
  ...
  // ✅ Close modal on anything with data-close
  ...
  // ✅ Gallery: clicking photo OR "View Details" opens modal
  document.getElementById("gallery-grid")?.addEventListener("click", (e) => {
    ...
    e.preventDefault();
    e.stopPropagation();
    openPropertyModal(Number(openBtn.dataset.openModal));
  });
